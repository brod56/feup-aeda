...
bool Client::operator<(const Client& c1) const {
    if (this->getVisitedProperties().empty()) return true;
    if (c1.getVisitedProperties().empty()) return false;
    int thisPrice = std::stoi(std::get<3>(getVisitedProperties().back()));
    int c1Price = std::stoi(std::get<3>(c1.getVisitedProperties().back()));
	return thisPrice >= c1Price;
}
